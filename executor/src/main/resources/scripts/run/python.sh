...
# Copy the Python script to the sandbox directory
if ! cp "$path_to_python_file" "/var/local/lib/isolate/$box_id/box/script.py"; then
    echo "{\"status\": \"Failed to copy Python script to sandbox\", \"error\": \"\", \"output\": \"\", \"isTLE\": false, \"isMLE\": false, \"cpu_time\": \"\", \"memory_used\": \"\", \"output_match\": \"not applicable\", \"wall_time\": \"\", \"exit_code\": \"\"}"
...
# Execute the Python script within the sandbox
if ! isolate --cg -b $box_id -t $time_limit --cg-mem $memory_limit -w $wall_time -M "/var/local/lib/isolate/$box_id/box/meta.txt" -o "output.txt" --stderr "error.txt" $stdin_option --run -- /usr/bin/python3 /box/script.py; then
    ...
    isTLE=$(echo "$metadata" | grep -q 'status:TO' && echo "true" || echo "false")
    memory_used=$(echo "$metadata" | grep 'max-rss:' | cut -d':' -f2)
    isMLE=$(echo "$metadata" | grep -q 'status:RE' && grep -q 'exitsig:9' && echo "true" || echo "false")
    ...
    cpu_time=$(echo "$metadata" | grep 'time:' | cut -d':' -f2)
    wall_time=$(echo "$metadata" | grep 'time-wall:' | cut -d':' -f2)
    exit_code=$(echo "$metadata" | grep 'exitcode:' | cut -d':' -f2)
